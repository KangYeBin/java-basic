...
public class HashSetExample {
	...
	public static void main(String[] args) {
		...
		/*
			...
			- 객체의 주소값을 할당하거나 암호화 알고리즘에서 많이 사용됨
		*/
		...
		/*
			- set은 인덱스가 없기 때문에 반복자 (Iterator)를 통해서 요소를 하나씩 꺼내야한다
			반복자 객체는 set 인터페이스가 제공하는 iterator()를 호출하여 얻어옴
		*/
		...
		/*
			...
			next()를 요소의 개수보다 많이 호출하게 되면 NoSuchElement 예외 발생

			hasNext() -> 반복자가 가지고 있는 객체를 더 가지고 올 수 있는지 여부를 확인하는 메서드를 통해 next()를 호출
		*/
		...
		// iterator를 향상 for문으로 대체
		for (String s : set) {
			System.out.println(s);
		}

		// remove(객체) : set에서 객체를 삭제, 인덱스가 없으므로 직접 지정
		set.remove("JSP");
		System.out.println(set);
